...
The semantic palette is unusual for a brand-marketing site it ships the full Apple Human Interface Guidelines accent ramp — `{colors.accent}` (Apple Blue `#007aff`), `{colors.danger}` (`#ff3b30`), `{colors.warning}` (`#ff9f0a`), `{colors.success}` (`#30d158`) plus their hoveractive deepenings — even though the marketing surfaces themselves only use these colors in the dark hero TUI mockup as syntax-highlight stand-ins. The wider palette belongs to the in-product TUI; the marketing pages mostly stay in monochrome.
...
### Spacing System
- Base unit 8px (with finer 124px steps available for tight inline gaps).
...
- Chart tiles — three thin-line ASCII charts inside the home page's open source AI coding agent stat block, with abstract dottedsparse-line plots in `{colors.body}` against the cream canvas. Captions sit beneath in `{typography.caption-md}` (`Fig 1. 150K GitHub Stars`, `Fig 2. 850 Contributors`, `Fig 3. 6.5M Monthly Devs`).
...
## Components
 ...
 No hover states documented per system policy. Each spec covers Default and ActivePressed only.
...
`badge-news` — small dark chip in the newsannouncement strip
...
- Contains the literal `curl -fsSL httpsopencode.aiinstall  bash` command with a small copy-icon at the right edge. Sits below the install-method tab strip.
...
`list-row` — featurebenefit row with ASCII bracket bullet
...
- Each row leads with `+`  `−` ASCII markers indicating expandcollapse state. Always rendered as plain text rows — no chevron icons, no animated accordion chrome.
...
- Contains an inline SVGCSS-drawn ASCII-style sparse-line plot (dotted, abstract — never specific data points) with a `Fig N. stat label` caption beneath in `{colors.mute}`.
...
- `enterprise` two-column layout desktop 5050 → tablet stacks to single-column with the form below the text block.
...
3. Run `npx @googledesign.md lint DESIGN.md` after edits — `broken-ref`, `contrast-ratio`, and `orphaned-tokens` warnings flag issues automatically.
