...
<template>
  <section>
    ...
    <form class="card mb-5 flex gap-2 p-4" @submit.prevent="load">
      <input v-model="query" class="field" aria-label="이름 또는 사번 검색" placeholder="이름 또는 사번" />
    ...
    <div class="card overflow-x-auto p-0">
      <table class="w-full min-w-[850px] text-left text-sm">
        ...
        <tbody>
          ...
          <tr v-for="employee in employees" v-else :key="employee.id" class="border-b border-slate-100 hover:bg-blue-50/40">
            <td class="p-4 font-semibold text-blue-700"><RouterLink :to="`/admin/employees/${employee.id}`">{{ employee.employeeNumber }}</RouterLink></td>
